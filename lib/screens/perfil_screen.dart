...
class _PerfilScreenState extends State<PerfilScreen> {
  ...
  Future<void> _alterarFoto() async {
    ...
    if (pickedFile != null) {
      ...
      final savedImage = await File(
        pickedFile.path,
      ).copy('${directory.path}/profile_$fileName');
      setState(() {
  ...
  void _salvar() {
    ...
    Provider.of<TreinoService>(
      context,
      listen: false,
    ).atualizarPerfil(nome, idade, altura, peso, _novoFotoPath);
  ...
  Widget build(BuildContext context) {
    final currentAltura =
        double.tryParse(_alturaController.text.replaceAll(',', '.')) ?? 1.70;
    final currentPeso =
        double.tryParse(_pesoController.text.replaceAll(',', '.')) ?? 70.0;
              ...
              child: Stack(
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage: _novoFotoPath != null
                        ? FileImage(File(_novoFotoPath!))
                        : null,
                    child: _novoFotoPath == null
                        ? const Icon(
                            Icons.person,
                            size: 60,
                            color: Colors.white,
