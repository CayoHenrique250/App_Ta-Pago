...
class _CadastroTreinoScreenState extends State<CadastroTreinoScreen> {
  ...
  void toggleDia(int dia) {
    ...
    // Verifica se o dia já está sendo usado em OUTRO treino
    String? treinoExistenteNome;
    
    for (var treino in service.listaDeTreinos) {
      // Se estamos editando, ignora o próprio treino
      if (widget.treinoParaEditar != null && treino.id == widget.treinoParaEditar!.id) continue;
    ...
    if (treinoExistenteNome != null && !diasSelecionados.contains(dia)) {
      // Se tentar marcar um dia ocupado, avisa e não marca
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("O dia já está ocupado pelo treino '$treinoExistenteNome'."),
    ...
    setState(() {
      diasSelecionados.contains(dia) ? diasSelecionados.remove(dia) : diasSelecionados.add(dia);
    });
  }

  void salvarTreino() {
    if (_nomeTreinoController.text.isEmpty || diasSelecionados.isEmpty || listaExercicios.isEmpty) {
  ...
  // NOVA FUNÇÃO: Dialog com seletor de imagem da galeria
  void mostrarDialogoAdicionarExercicio() {
      ...
      File? imagemSelecionada; // Variável temporária para o dialog

      showDialog(
        context: context,
        builder: (ctx) => StatefulBuilder( // StatefulBuilder para atualizar o dialog ao escolher foto
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: imagemSelecionada != null 
                            ? DecorationImage(image: FileImage(imagemSelecionada!), fit: BoxFit.cover)
                            : null
                        ),
                        child: imagemSelecionada == null 
                          ? Column(
                              ...
                              children: const [
                                ...
                                Text("Toque para add Imagem", style: TextStyle(color: Colors.white70))
                              ],
                            )
                          : null,
                      ),
                    ...
                    TextField(controller: nomeExController, decoration: const InputDecoration(labelText: 'Nome')),
                    Row(children: [
                      Expanded(child: TextField(controller: seriesController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Séries'))),
                    ...
                    TextField(controller: pesoController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Carga (kg)')),
                  ],
                ),
              ),
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    if (nomeExController.text.isNotEmpty) {
                      setState(() {
                        listaExercicios.add(ExercicioModelo(
                          id: const Uuid().v1(),
                          nome: nomeExController.text,
                          series: seriesController.text,
                          repeticoes: repsController.text,
                          peso: pesoController.text,
                          imageUrl: imagemSelecionada?.path, // SALVA O CAMINHO LOCAL
                        ));
                      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.treinoParaEditar == null ? "Novo Treino" : "Editar Treino")),
      body: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          children: [
            TextField(controller: _nomeTreinoController, decoration: const InputDecoration(labelText: 'Nome do Treino', border: OutlineInputBorder())),
            const SizedBox(height: 10),
            ...
            Row(
              ...
              children: [
                ...
                IconButton(onPressed: mostrarDialogoAdicionarExercicio, icon: Icon(Icons.add_circle, color: Theme.of(context).colorScheme.primary)),
              ],
            ),
            Expanded(
                ...
                itemBuilder: (ctx, index) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text("${ex.series}x${ex.repeticoes} - ${ex.peso}kg"),
                      trailing: IconButton(
                        icon: const Icon(Icons.delete, color: Colors.red),
                        onPressed: () => setState(() => listaExercicios.removeAt(index)),
                      ),
                    ),
                  );
                },
            ...
            SizedBox(
              width: double.infinity,
              height: 50,
              child: ElevatedButton(onPressed: salvarTreino, child: const Text("SALVAR")),
            )
          ],
        ),
      ),
